  ...
  const warnings = [
    {
      ...
      food: [
        '펠렛과 알곡 혼합',
        '과일 / 채소 / 견과 / 무염국수 및 파스타 추가급여 가능\n(급여를 다양하게 해주시는 것이 좋습니다)',
      ],
      life: [
        '울음소리가 비교적 큰 동물이기에 분양 시 고려해주세요',
        "우관을 정리하기 위해 '파우더'라는 흰 가루가 소량 발생합니다",
        '외로움을 잘 타는 친구들이기에 주인분과 함께할 시간이 늘 필요해요',
      ],
    },
  ];
  ...
  return (
    <div className={Style.naturaHouseContainer}>
      ...
      <div className={Style.banner}>
        <Slider {...settings}>
          <img src={Img01} />
          <img src={ContentImg01} />
          <img src={ContentImg02} />
        </Slider>
      </div>
        ...
        {shoplist.map((index) => (
          <div className={Style.naturaHouseContent}>
            <div className={Style.nameContainer}>
              <div className={Style.nameContainer2}>
                <div id={Style.header}>
                  <span id={Style.species}>{index.species}</span>
                  <div id={Style.store}>{index.store}</div>
                </div>
                <Link to="../../chat" id={Style.border}>
                  <img src={charIcon} alt="chatIcon" className={Style.chatIcon} />
                </Link>
              </div>
            </div>
        ...
        <div className={Style.productContainer}>
          {productlist.map((index) => (
            <div className={Style.product}>
              <img src={index.img} alt="productImg" className={Style.productImg} />
      ...
      <div className={Style.warningsContainer}>
        ...
        {warnings.map((index) => (
          <div className={Style.warning}>
            ...
            <div className={Style.warningLife}>
              *생활
              {index.life.map((life) => (
